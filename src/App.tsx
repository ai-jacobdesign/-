...
// 4. Logo component using the official Charm Medical Center vector logo
export function CharmLogo({ 
  ...
  if (variant === "symbol") {
    return (
      ...
      >
        <defs>
          <linearGradient id={`charm-cyan-blue-${id}`} x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#38bdf8" />
            <stop offset="40%" stopColor="#0284c7" />
            <stop offset="100%" stopColor="#0369a1" />
          </linearGradient>
          <linearGradient id={`charm-dark-blue-${id}`} x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        {/* Outer C Arc */}
        <path 
          d="M 98 22 C 72 8, 32 16, 16 48 C 0 80, 12 108, 48 116 C 76 122, 104 112, 114 98 C 96 108, 68 108, 48 98 C 30 88, 22 68, 30 46 C 38 24, 66 18, 98 22 Z" 
          fill={`url(#charm-cyan-blue-${id})`} 
        />
        {/* Bottom Swoosh */}
        <path 
          d="M 18 68 C 12 88, 24 110, 54 116 C 82 122, 108 112, 118 96 C 96 108, 66 110, 46 98 C 30 88, 22 72, 18 68 Z" 
          fill={`url(#charm-dark-blue-${id})`} 
        />
        {/* Inner Cross */}
        <g transform="translate(52, 42)">
          <rect x="15" y="0" width="18" height="50" rx="6" fill="#1d4ed8" />
          <rect x="0" y="16" width="48" height="18" rx="6" fill="#1d4ed8" />
        </g>
      </svg>
    );
  }
  ...
  return (
    ...
    >
      ...
      <g transform="translate(0, 0)">
        <path 
          d="M 88 18 C 64 6, 28 12, 14 42 C 0 72, 10 98, 42 104 C 68 110, 94 100, 102 88 C 86 96, 60 96, 42 88 C 26 80, 20 62, 26 42 C 32 22, 58 16, 88 18 Z" 
          fill={`url(#charm-cyan-blue-${id})`} 
        />
        <path 
          d="M 16 60 C 10 78, 20 98, 48 104 C 74 110, 98 100, 106 86 C 86 96, 58 98, 40 88 C 26 80, 20 66, 16 60 Z" 
          fill={`url(#charm-dark-blue-${id})`} 
        />
        <g transform="translate(46, 36)">
          <rect x="13" y="0" width="16" height="44" rx="5" fill="#1d4ed8" />
          <rect x="0" y="14" width="42" height="16" rx="5" fill="#1d4ed8" />
        </g>
      </g>

      {/* Typography matching 사)누가 참의원 Charm Medical Center */}
      <g transform="translate(125, 0)">
        <text x="0" y="22" fontFamily="'Apple SD Gothic Neo', 'Noto Sans KR', 'Malgun Gothic', sans-serif" fontWeight="700" fontSize="18" fill="#1e3a8a">사) 누가</text>
        <text x="0" y="72" fontFamily="'Apple SD Gothic Neo', 'Noto Sans KR', 'Malgun Gothic', sans-serif" fontWeight="900" fontSize="48" fill="#1e3a8a" letterSpacing="-1.5">참의원</text>
        <text x="2" y="102" fontFamily="Arial, 'Helvetica Neue', sans-serif" fontWeight="700" fontSize="22" fill="#1e3a8a" letterSpacing="-0.5">Charm Medical Center</text>
      </g>
    </svg>
  );
}
...
export default function App() {
  ...
  const handleDownloadMealPlanAsImage = async () => {
    ...
    try {
      ...
      downloadLink.download = `참의원_투석환자_식단표_${patientName}_${patientId}.png`;
      downloadLink.click();
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans antialiased pb-20">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            {/* CharmLogo using uploaded logo image */}
            <CharmLogo className="h-12 w-auto flex-shrink-0" />
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs font-bold px-2 py-0.5 bg-blue-50 text-blue-800 rounded-md border border-blue-200">
                  참의원
                </span>
      ...
      */}
      <div className="absolute -left-[9999px] -top-[9999px] overflow-hidden" style={{ width: '1122px' }}>
        ...
        >
          {/* Export Header */}
          <div className="bg-white border-2 border-blue-600 rounded-xl p-3 flex justify-between items-center shrink-0">
            <div className="flex items-center gap-4">
              <CharmLogo className="h-16 w-auto flex-shrink-0" />
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[12px] bg-blue-800 text-white font-extrabold px-2 py-0.5 rounded-sm uppercase tracking-wide">
                    Clinical Dialysis Diet
                  </span>
                  <span className="text-[14px] text-slate-500 font-bold font-mono">Charm Medical Center</span>
                </div>
                <h1 className="text-[26px] font-extrabold text-slate-900 tracking-tight leading-none mb-2">
                  참의원 인공신장실 맞춤형 투석 환자 4주 건강 식단표
                </h1>
          ...
          <div className="text-[13px] text-slate-400 font-semibold border-t border-slate-200 pt-2 flex justify-between items-center shrink-0">
            <span>본 식단표는 대한신장학회 지침 및 참의원 인공신장실 의료 자문을 바탕으로 구성된 안심 식단 가이드라인입니다.</span>
